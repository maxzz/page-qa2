...
export function JsonBeautifier() {
    ...
    useEffect(() => {
        runTest(perLine, setText, setFormatted, setNLines);
    }, [perLine]);
                ...
                <div className="">
                    ...
                    {!!text.length &&
                        <div className="relative py-0.5 min-h-[2.6rem] bg-slate-200 rounded">
                            {/* Text area */}
                            <div className="px-2 max-h-115 text-[.75rem] whitespace-pre overflow-auto">
                                {formatted}
                            </div>

                            {/* Buttons */}
                            <div className="absolute top-0.5 right-1 p-1 bg-slate-200 flex space-x-0.5">
                                <div className="px-2 py-1.5 text-xs border-slate-100 border rounded shadow-sm">
                                    {nLines} line{nLines != 1 ? 's' : ''}
                                </div>

                                <button className="px-2 py-1.5 text-xs border-slate-400 border rounded shadow-sm" onClick={() => { setText(''); }}>
                                    Clear
                                </button>

                                <button className="px-2 py-1.5 text-xs border-slate-400 border rounded shadow-sm" onClick={async () => { await navigator.clipboard.writeText(formatted); }}>
                                    Copy
                                </button>
                            </div>
                        </div>
...
function runTest(perLine: number, setText: (t: string) => void, setFormatted: (f: string) => void, setNLines: (n: number) => void) {
