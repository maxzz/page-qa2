    ...
    return (
        <div className="cursor-default">
            <div className="flex items-center space-x-1">
                <div className="mb-1 pl-3 text-sm font-bold">{`${convTBrowserShort2Name(browser)}`}</div>
            ...
            <div className="py-1 grid grid-cols-3 bg-slate-100 shadow">
                ...
                {table.map(
                    (item, idx) => (
                        <Fragment key={idx}>
                            <div className={`text-xs ${idx ? 'opacity-25' : ''} ${!idx ? 'pt-0.5' : ''}`}><div className="px-3 pt-0.5">{convTBrand2Name(item.brand)}</div></div>
